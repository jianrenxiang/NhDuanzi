...
- (void)searchWithText:(NSString *)text {
    ...
    [userRequest nh_sendRequeseWithCompletion:^(id response, BOOL success, NSString *message) {
        if (success) {
    ...
    [contentRequest nh_sendRequeseWithCompletion:^(id response, BOOL success, NSString *message) {
        if (success) {
    ...
    [cetegoryRequest nh_sendRequeseWithCompletion:^(id response, BOOL success, NSString *message) {
        if (success) {
...
- (NHBaseTableViewCell *)nh_cellAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.isFindMoreFriendsFlag) {
        NHDiscoverSearchFriendUserCell *cell = [NHDiscoverSearchFriendUserCell cellWithTabelView:self.tableView];
    ...
    if (section == 0) {
        NHDiscoverTableViewCell *cell = [NHDiscoverTableViewCell cellWithTabelView:self.tableView];
        [cell setElementModel:self.hotCategoryArray[indexPath.row] keyWord:self.keyWord];
        return cell;
    } else if (section == 1) {
        WeakSelf(weakSelf);
        NHDiscoverSearchFriendCell *cell = [NHDiscoverSearchFriendCell cellWithTabelView:self.tableView];
        ...
        NHHomeTableViewCell *cell = [NHHomeTableViewCell cellWithTabelView:self.tableView];
