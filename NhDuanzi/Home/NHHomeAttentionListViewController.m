...
- (void)loadData {
    if (self.request) {
        [self.request nh_sendRequeseWithCompletion:^(id response, BOOL success, NSString *message) {
            if (success) {
                NSDictionary *dict = (NSDictionary *)response;
                if ([dict.allKeys containsObject:@"users"]) {
...
- (NHBaseTableViewCell *)nh_cellAtIndexPath:(NSIndexPath *)indexPath {
    NHHomeAttentionListTableViewCell *cell = [NHHomeAttentionListTableViewCell cellWithTabelView:self.tableView];
